...
enum abstract DialoguePosition(String) to String
{
	var LEFT = 'left';
	var CENTER = 'center';
	var RIGHT = 'right';
...
class DialogueCharacter extends FlxSprite
{
	...
	var bounceTwn:Null<FlxTween> = null;
	
	public function bounce(up:Bool = true)
	{
		bounceTwn?.cancel();
		
		bounceTwn = FlxTween.tween(this, {'offset2.y': up ? -10 : 0}, 0.1,
			{
				ease: FlxEase.sineIn,
				onComplete: Void -> {
					if (up) bounce(false);
