...
class Character extends FlxSprite
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		final pitch = PlayState.instance?.inst?.pitch ?? FlxG.sound.music?.pitch ?? 1.0;
		
		if (!isPlayer && holdTimer >= Conductor.stepCrochet * (0.0011 #if FLX_PITCH / pitch #end) * singDuration)
		{
			dance();
			holdTimer = 0;
		}
	...
	var __previousAnim:String = '';
	
	var animCurFrame(get, set):Int;
	
	function set_animCurFrame(v:Int):Int
	{
		if (isAnimationNull()) return 0;
		
		if (isAnimateAtlas) atlas.animation.curAnim.curFrame = v;
		else animation.curAnim.curFrame = v;
	...
	function get_animCurFrame():Int
	{
		...
		return isAnimateAtlas ? atlas.animation.curAnim.curFrame : animation.curAnim.curFrame;
	}
	
	inline public function getAnimationName():String
	{
		return __previousAnim;
	}
