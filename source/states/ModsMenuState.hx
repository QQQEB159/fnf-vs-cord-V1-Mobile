...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bgButtons = FlxSpriteUtil.drawRoundRectComplex(new FlxSprite(bgDescription.x, bgDescription.y + bgDescription.height - myHeight).makeGraphic(840, myHeight, FlxColor.TRANSPARENT), 0, 0, 840,
		...
		if (modsGroup.members[curSelectedMod].settings == null
			|| modsGroup.members[curSelectedMod].settings.length < 1) settingsButton.enabled = false;
				...
				if (hoveringOnMods)
				{
						...
						|| FlxG.gamepads.anyJustPressed(RIGHT_TRIGGER))
					{
						if (FlxG.keys.justPressed.END
							|| FlxG.gamepads.anyJustPressed(RIGHT_TRIGGER)) curSelectedMod = modsList.all.length - 1;
					...
					else if (controls.UI_UP || controls.UI_DOWN)
					{
						...
						holdTime += elapsed;
						if (holdTime > 0.5
							&& Math.floor(lastHoldTime * 8) != Math.floor(holdTime * 8)) changeSelectedMod(shiftMult * (controls.UI_UP ? -1 : 1));
					}
	...
	function updateModDisplayData()
	{
		...
		for (button in buttons)
			if (button.focusChangeCallback != null) button.focusChangeCallback(button.onFocus);
...
class MenuButton extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!ignoreCheck && !Controls.instance.controllerMode && FlxG.mouse.justMoved && FlxG.mouse.visible) onFocus = FlxG.mouse.overlaps(this);
		...
		{
			_needACheck = false;
			if (!Controls.instance.controllerMode) setButtonVisibility(FlxG.mouse.overlaps(this));
		}
	}
