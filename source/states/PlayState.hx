...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Ur actually ass', 0.2], // From 0% to 19%
		['Yucky', 0.4], // From 20% to 39%
		['Ew', 0.5], // From 40% to 49%
		['Nah', 0.6], // From 50% to 59%
		['Eh', 0.69], // From 60% to 68%
		['Please become funny', 0.7], // 69%
	...
	public var usesShaders:Bool = true;
	...
	override public function create()
	{
		...
		usesShaders = ClientPrefs.data.shaders;
		...
		if (gf != null)
		{
			camPos.x += gf.getMidpoint().x + gf.cameraPosition[0];
			camPos.y += gf.getMidpoint().y + gf.cameraPosition[1];
		}
		...
		uiGroup.add(timeTxt);
		timeTxt.text = "- " + SONG.song + " [" += storyDifficultyText.toUpperCase() + "] -";
		
		noteUnderlay = new FlxSprite(Paths.image('notesbar'));
		noteUnderlay.alpha = 0;
		noteGroup.add(noteUnderlay);
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return currentHealth, 0, 2);
		...
		/* botplayTxt = new FlxText(400, timeTxt.y + 55, FlxG.width - 800, "BOTPLAY", 32);
			...
			if (ClientPrefs.data.downScroll) botplayTxt.y = timeTxt.y - 78; */
	...
	public function startVideo(name:String)
	{
		...
		var videoBg = new FlxBGSprite();
		videoBg.color = FlxColor.BLACK;
		add(videoBg);
		videoBg.cameras = [camOther];
		
		FunkinVideoSprite.init();
		
		var video = new FunkinVideoCutscene(true);
		add(video);
		video.load(filepath);
		video.onFormat(() -> {
		...
		video.onEnd(() -> {
			video.kill();
			videoBg.kill();
			if (PlayState.instance != null) startAndEnd();
		}, true);
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if (dialogueFile.dialogue.length > 0)
		{
			...
			if (endingSong)
			{
				psychDialogue.onComplete = function() {
					psychDialogue.kill();
	...
	dynamic function updateUnderlay()
	{
		final spr = playerStrums.getFirst(spr -> true);
		if (spr != null && noteUnderlay != null)
		{
			FlxTween.cancelTweensOf(noteUnderlay, ['alpha']);
			noteUnderlay.x = spr.x - 7;
			noteUnderlay.alpha = ClientPrefs.data.noteUnderlayAlpha;
			
			noteUnderlay.flipY = ClientPrefs.data.downScroll;
		}
	}
	...
	public function startCountdown()
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				{
					case 0:
						final sound = Paths.sound('intro3' + introSoundsSuffix);
						if (sound != null) FlxG.sound.play(sound, 0.6);
						tick = THREE;
					case 1:
						final sound = Paths.sound('intro2' + introSoundsSuffix);
						if (sound != null) FlxG.sound.play(sound, 0.6);
						
						countdownReady = createCountdownSprite(introAlts[0], antialias);
						tick = TWO;
					case 2:
						final sound = Paths.sound('intro1' + introSoundsSuffix);
						if (sound != null) FlxG.sound.play(sound, 0.6);
						
						countdownSet = createCountdownSprite(introAlts[1], antialias);
						tick = ONE;
					case 3:
						final sound = Paths.sound('introGo' + introSoundsSuffix);
						if (sound != null) FlxG.sound.play(sound, 0.6);
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		else if (!ClientPrefs.data.simplifiedScore)
		{ // complicated score
			scoreTxt.text = 'Accuracy:'
				+ (ratingName != 'N/A' ? ' ${CoolUtil.floorDecimal(ratingPercent * 100, 2)}%'
					+ ' • Misses: '
					+ songMisses
					+ ' • Rank: '
					+ ratingName
	...
	function startSong():Void
	{
		...
		FlxTween.tween(timeTxt, {alpha: 0.7}, 0.5, {ease: FlxEase.circOut});
	...
	function eventEarlyTrigger(event:EventNote):Float
	{
		...
		if (returnedValue == null) return 0;
		
		if (returnedValue is String) returnedValue = Std.parseFloat(returnedValue);
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (!isStoryMode && !skipArrowStartTween)
			{
				// babyArrow.y -= 10;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {/*y: babyArrow.y + 10,*/ alpha: targetAlpha}, 0.75, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
			}
	...
	{
		#if DISCORD_ALLOWED
		if (health > 0 && !paused && autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, null);
		#end
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (showTime) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
		else DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, null);
		#end
	}
	...
	{
		currentHealth = FlxMath.lerp(currentHealth, health, 1 - Math.exp(-elapsed * 9));
		
		if (FlxG.keys.justPressed.NINE) // swag
			iconP1.swapOldIcon();
			
		if (!inCutscene && !paused && !freezeCamera)
		{
			// psych 0.7 stopped using camFollowPos to use followlerp why is this back ?
			FlxG.camera.followLerp = 0.1;
			final rate = 3 * cameraSpeed * playbackRate;
			camFollowPos.setPosition(CoolUtil.decayLerp(camFollowPos.x, camFollow.x, rate, elapsed), CoolUtil.decayLerp(camFollowPos.y, camFollow.y, rate, elapsed));
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(FlxG.camera.zoom, defaultCamZoom, 1 - Math.exp(-elapsed * 3.125 * camZoomingDecay * playbackRate));
			camHUD.zoom = FlxMath.lerp(camHUD.zoom, 1, 1 - Math.exp(-elapsed * 3.125 * camZoomingDecay * playbackRate));
		}
	...
	public dynamic function updateIconsPosition(elapsed:Float, snapped:Bool = false)
	{
		...
		else
		{
			...
			iconP2.x = healthBar.barCenter - (190) / 2 - iconOffset;
		}
	}
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		iconP1.animation.curAnim.curFrame = (healthBar.percent < 30) ? 1 : 0; // If health is under 20%, change player icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		iconP2.animation.curAnim.curFrame = (healthBar.percent > 70) ? 1 : 0; // If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		return health;
	}
	
	function openPauseMenu()
	{
		FlxG.camera.followLerp = 0; // FUCK. YOU.
		...
		#if DISCORD_ALLOWED
		if (autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, null);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				if (autoUpdateRPC) DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, null);
				#end
				isDead = true;
				return true;
			}
	...
	public function getCharacterCameraPos(char:Character):FlxPoint
	{
		...
		{
			desiredPos.x += 150 + char.cameraPosition[0];
		}
	...
	public function getSingDisplacement(char:Character):FlxPoint
	{
		if (char == null) return FlxPoint.weak();
		
		final anim = char.getAnimationName().toLowerCase();
		
		if (anim.contains('up')) return FlxPoint.weak(0, -camDisplacement);
		else if (anim.contains('down')) return FlxPoint.weak(0, camDisplacement);
		else if (anim.contains('left')) return FlxPoint.weak(-camDisplacement, 0);
		else if (anim.contains('right')) return FlxPoint.weak(camDisplacement, 0);
		else return FlxPoint.weak();
	...
	{
		final curCharacter:Null<Character> = isDad ? dad : boyfriend;
		
		// if (camCurTarget != null) curCharacter = camCurTarget;
		
		final desiredPos = getCharacterCameraPos(curCharacter);
		
		camFollow.x = desiredPos.x;
		camFollow.y = desiredPos.y;
		
		final displacement = getSingDisplacement(curCharacter);
	...
	public function endSong()
	{
		...
		checkForAchievement([weekNoMiss, 'pRank', 'goldenPRank', '20percent']);
		#end
		
		var ret:Dynamic = callOnScripts('onEndSong', null, true);
		if (ret != FunkinLua.Function_Stop && !transitioning)
		{
			saveAllImportantStats();
			...
			else
			{
				...
				MusicBeatState.currentTransition = STICKERS;
				if (songName.toLowerCase() == 'catch') MusicBeatState.currentTransition = WEB_FISHING;
	...
	public function saveProgression(?song:String)
	{
		var percent:Float = ratingPercent;
		if (Math.isNaN(percent)) percent = 0;
		Highscore.saveScore(song ?? SONG.song, songScore, storyDifficulty, percent);
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length == 0 && !ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay'))
			{
	...
	public function saveAllImportantStats()
	{
		...
		Stats.instance.songsPlayed++;
		Stats.instance.totalScore += songScore;
		
		Stats.instance.totalSicks += sicks;
		Stats.instance.totalGoods += goods;
		Stats.instance.totalBads += bads;
		Stats.instance.totalShits += shits;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		score = scoreNoteFUNKIN(noteDiff);
		...
		if (!PlayState.isPixelStage)
		{
			...
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 0.1));
		}
		else
		{
	...
	// thank you funkin
	public static final FUNKIN_MAX_SCORE:Int = 500;
	public static final FUNKIN_SCORING_OFFSET:Float = 54.99;
	public static final FUNKIN_SCORING_SLOPE:Float = 0.080;
	public static final FUNKIN_MIN_SCORE:Float = 9.0;
	public static final FUNKIN_MISS_SCORE:Int = 0;
	public static final FUNKIN_PERFECT_THRESHOLD:Float = 5.0; // 5ms
	public static final FUNKIN_MISS_THRESHOLD:Float = 160.0;
	
	static function scoreNoteFUNKIN(msTiming:Float):Int
	{
		...
		{
			case(_ > FUNKIN_MISS_THRESHOLD) => true:
				FUNKIN_MISS_SCORE;
			case(_ < FUNKIN_PERFECT_THRESHOLD) => true:
				FUNKIN_MAX_SCORE;
			default:
				// Fancy equation.
				var factor:Float = 1.0 - (1.0 / (1.0 + Math.exp(-FUNKIN_SCORING_SLOPE * (absTiming - FUNKIN_SCORING_OFFSET))));
				
				var score:Int = Std.int(FUNKIN_MAX_SCORE * factor + FUNKIN_MIN_SCORE);
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (result != FunkinLua.Function_Stop
			&& result != FunkinLua.Function_StopHScript
			&& result != FunkinLua.Function_StopAll) callOnHScript('noteMiss', [daNote]);
	...
	public function healthbarShake(intensity:Float)
	{
		new FlxTimer().start(0.01, function(tmr:FlxTimer) {
			iconP1.y += ((10 * intensity) / 2);
			iconP2.y += ((10 * intensity) / 2);
			healthBar.y += ((10 * intensity) / 2);
		});
		new FlxTimer().start(0.05, function(tmr:FlxTimer) {
			iconP1.y -= ((15 * intensity) / 2);
			iconP2.y -= ((15 * intensity) / 2);
			healthBar.y -= ((15 * intensity) / 2);
		});
		new FlxTimer().start(0.10, function(tmr:FlxTimer) {
			iconP1.y += ((8 * intensity) / 2);
			iconP2.y += ((8 * intensity) / 2);
			healthBar.y += ((8 * intensity) / 2);
		});
		new FlxTimer().start(0.15, function(tmr:FlxTimer) {
			iconP1.y -= ((5 * intensity) / 2);
			iconP2.y -= ((5 * intensity) / 2);
			healthBar.y -= ((5 * intensity) / 2);
		});
		new FlxTimer().start(0.20, function(tmr:FlxTimer) {
			iconP1.y += ((3 * intensity) / 2);
			iconP2.y += ((3 * intensity) / 2);
			healthBar.y += ((3 * intensity) / 2);
		});
		new FlxTimer().start(0.25, function(tmr:FlxTimer) {
			iconP1.y -= ((1 * intensity) / 2);
			iconP2.y -= ((1 * intensity) / 2);
			healthBar.y -= ((1 * intensity) / 2);
		});
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (result != FunkinLua.Function_Stop
			&& result != FunkinLua.Function_StopHScript
			&& result != FunkinLua.Function_StopAll) callOnHScript('opponentNoteHitPre', [note]);
		...
		if (result != FunkinLua.Function_Stop
			&& result != FunkinLua.Function_StopHScript
			&& result != FunkinLua.Function_StopAll) callOnHScript('opponentNoteHit', [note]);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (result != FunkinLua.Function_Stop
			&& result != FunkinLua.Function_StopHScript
			&& result != FunkinLua.Function_StopAll) callOnHScript('goodNoteHitPre', [note]);
		...
		if (!note.isSustainNote)
		{
			...
			if (combo > Stats.instance.bestCombo) Stats.instance.bestCombo = combo;
		...
		if (result != FunkinLua.Function_Stop
			&& result != FunkinLua.Function_StopHScript
			&& result != FunkinLua.Function_StopAll) callOnHScript('goodNoteHit', [note]);
		if (!note.isSustainNote) invalidateNote(note);
	}
	...
	public function playerDance():Void
	{
		...
		if (boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 #if FLX_PITCH / inst.pitch #end) * boyfriend.singDuration && anim.startsWith('sing') && !anim.endsWith('miss')) boyfriend.dance();
	}
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		for (name in achievesToCheck)
		{
			...
			if (name != WeekData.getWeekFileName() + '_nomiss' && Achievements.exists(name)) // common achievements
			{
				...
				{
					case 'pRank':
						unlock = (songMisses == 0 && !practiceMode);
					case 'goldenPRank':
						unlock = (ratingPercent >= 1 && !practiceMode);
					case '20percent':
						unlock = (ratingPercent < 0.2 && !practiceMode);
						// case 'miss100':
						// unlock = (songMisses >= 100 && !practiceMode);
