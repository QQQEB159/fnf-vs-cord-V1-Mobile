...
class CharacterEditorState extends MusicBeatState
{
	...
	function makeUIMenu()
	{
		...
		UI_box.forEach(spr -> {
			if (spr != null) spr.cameras = [camHUD];
		});
	}
	...
	function addGhostUI()
	{
			...
			if (!character.isAnimationNull())
			{
				...
				else if (myAnim != null) // This is VERY unoptimized and bad, I hope to find a better replacement that loads only a specific frame as bitmap in the future.
				{
					...
					if (animateGhost == null
						|| animateGhostImage != character.imageFile) Paths.loadAnimateAtlas(animateGhost, character.imageFile);
		...
		var ghostAlphaSlider:FlxUISlider = new FlxUISlider(this, 'ghostAlpha', 10, makeGhostButton.y + 25, 0, 1, 210, 15, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FlxUIDropDownMenuEx(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(index:String) {
	...
	function addCharacterUI()
	{
		...
		imageInputText = new FlxUIInputTextEx(15, 30, 200, character.imageFile, 8);
		...
		healthIconInputText = new FlxUIInputTextEx(15, imageInputText.y + 35, 75, healthIcon.getCharacter(), 8);
		...
		if (!character.isAnimationNull())
		{
			...
			if (FlxG.keys.justPressed.A || FlxG.keys.justPressed.D || holdingFrameTime > 0.5)
			{
				...
				if (holdingFrameTime <= 0.5 || holdingFrameElapsed > 0.1)
				{
					...
					else character.atlas.anim.curAnim.curFrame = frames;
